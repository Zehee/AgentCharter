...
11. **Discussion-loop five-step method** — When a discussion with the human produces a conclusion, follow this exact sequence: (1) **Explicitly summarize the conclusion and ask the human "Confirm?". Do NOT proceed until you receive an explicit confirmation word — "OK", "yes", "agreed", "好的". Never assume implicit consent or acquiescence.** (2) Immediately write a DECISION recording the reasoning chain. (3) Derive TASKs or TODOs from the DECISION's "Final Artifacts" table. (4) Execute each TASK, writing a REPORT upon completion. (5) Append the operation log to `logs/tpm-log.md`. No step may be skipped.
