...
NAME_PATTERNS: dict[str, str] = {
    ...
    "REVISION": r"^REVISION_\d{3}[A-Z]?_\d{8}_[A-Z]+@[A-Z]+\.md$",
...
TEMPLATE_BASE_PATTERNS: dict[str, str] = {
    ...
    "REPORT": r"^REPORT_NNN_DATE_[a-z]+@[a-z]+\.md$",
    "REVIEW_REPORT": r"^REVIEW_REPORT_NNN_DATE_[a-z]+@[a-z]+\.md$",
    ...
    "REVISION": r"^REVISION_NNN[A-Z]?_DATE_[a-z]+@[a-z]+\.md$",
...
def generate_filename(
    ...
    if file_type == "REPORT":
        return f"REPORT_{nnn}_{date}_{author}@{recipient}.md"

    if file_type == "REVIEW_REPORT":
        return f"REVIEW_REPORT_{nnn}_{date}_{author}@{recipient}.md"
    ...
    if file_type == "REVISION":
        return f"REVISION_{nnn}_{date}_{author}@{recipient}.md"
