...
11. **讨论闭环五步法** — 与人类讨论形成结论后，严格按顺序执行：(1) **显式总结结论，向人类提问"是否确认？"，必须得到"可以""同意""好的"等明确确认词后才能继续。** 不可以默认人类已同意或不置可否就跳过。(2) 立即写 DECISION 记录推理链。(3) 从 DECISION 的"最终产物"派生 TASK 或 TODO。(4) 逐个执行 TASK，完成后写 REPORT。(5) 追加操作日志到 `logs/tpm-log.md`。每一步不可跳过。
...
- **段间用 `_`**，**段内用 `-`**
- `NNN`: 任务序号，如 `043`、`049C`。`_R1`、`_R2` 是独立的轮次段，不属于 NNN
