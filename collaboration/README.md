...
4. **节省上下文**：模板是固定格式，每次写文件都重读一遍浪费上下文。TPM 可以把高频模板（TASK、REPORT、REVIEW_REPORT）的结构记住，帮你直接生成文件。如果 TPM 没帮你，你还是得去 `templates/` 复制模板——无论哪种方式，最终文件必须符合模板格式和命名规范。本条是效率建议，非强制。
