...
11. **讨论闭环五步法** — 与人类讨论形成结论后，严格按顺序执行：(1) 总结结论，等人类确认。(2) 立即写 DECISION 记录推理链。(3) 从 DECISION 的"最终产物"派生 TASK 或 TODO。(4) 逐个执行 TASK，完成后写 REPORT。(5) 追加操作日志到 `logs/tpm-log.md`。每一步不可跳过。
