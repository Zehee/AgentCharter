...
### 实战案例：wolf-judge 项目的 DECISION_001

**来源**：wolf-judge 项目的 TPM（DSpro）与决策者的一次结对讨论。决策内容是"后端集成测试优先于端到端联调"——这是一个影响后续测试策略方向的基线决策。

**触发流程**：

1. **决策者出题**：DSpro 分析 TASK_014 场景 A 的四种子场景后，发现后端集成测试比端到端联调更紧迫
2. **DSpro 分析**：列出两种路径（先联调 vs 先后端集成测试），分析各自风险和收益
3. **决策者认可**：确认"后端集成测试优先"
4. **DSpro 提问**："是否需要我创建 DECISION_001 来记录这条决策？你来决定。"
5. **决策者确认**："好的，执行我们的决策"
6. **DSpro 创建 DECISION_001**：记录推理链（端到端联调会混入前端问题、测试可信度差异）、替代方案（手动 `tauri dev` 联调）、否决理由
7. **DSpro 创建 TASK_014**：inbox/ 中等待 Kimi 巡检领取
8. **DSpro 同步本地记忆**：将"有决策含义的对话必须写 DECISION"写入自己的持久记忆

**关键观察**：

- DSpro 在得到"好的"之前没有写任何文件——严格遵守"先确认，再行动"的原则
- DSpro 在对话中主动识别这是一个决策信号，不等决策者说"记下来"——符合"AI 必须主动识别"的触发原则
- DSpro 同时更新了自己的本地记忆，确保下次会话不会丢失这条规则——这是 TPM 和 External Agent 不使用 `context/` 目录、用自己的记忆系统的实战验证

**这段推理链在其他场景下会发生什么？**

| 参与者 | 变化 |
|--------|------|
| **决策者** | 三个月后看到 DECISION_001，立即知道"当时为什么放弃端到端先做后端测试" |
| **DSpro** | 上下文即使被压缩，重新注入本地记忆后恢复"决策必须写 DECISION"规则 |
| **Kimi 或其他执行 Agent** | 领取 TASK_014 时看到 `决策来源: DECISION_001`，理解任务背后的判断依据 |

**一句话**：DECISION 记录了"为什么"，TASK 记录"做了什么"，REPORT 记录"做完了"。三者缺一不可。
